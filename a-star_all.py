...
colours = [
    ...
    ,["red",(255,0,0)] #red
 	...
 	,["maroon (bordeaux)",(128,0,0)] #maroon (bordeaux)
 	,["olive",(128,128,0)] #olive
 	,["green",(0,128,0)] #green
 	,["purple",(128,0,128)] #purple
 	,["teal (pool blue)",(0,128,128)] #teal (pool blue)
 	,["navy (dark blue)",(0,0,128)] #navy (dark blue)
    ,["orange",(255,69,0)] #orange
    ,["gold",(255,215,0)] #gold
    ,["saddle brown",(255,20,147)] #saddle brown
...
def calc_heuristics(destination): #destination -> str
    for key in heuristic: 
        if (key != destination):
            ...
            heuristic[key][0] = round(dist*0.65,2) #1px ~ 0.65 km; convert the distance in pixels to a distance in kilometers
...
def main_function(colours, nodes, origin):
    stop = False #to stop the main while

    dest = "" #destination
    i_colour = -1 #index for the list of colours
    ...
    for node in nodes:
        ...
        if (node != origin):
            ...
            curr = nodes[origin] #the first node (and the current node for each iteration in the while loop)
            path.append(origin)
            curr_adj = curr["adj"]
            nodes_adj.append(origin)
            i_colour += 1

            print("Destination: "+ dest +", Colour: " + colours[i_colour][0])
        else:
            stop = True
            dist_acc = 0
        while not stop:
...
main_function(colours, nodes, origin)

cv2.namedWindow("image", cv2.WINDOW_NORMAL) #create window with freedom of dimensions
cv2.imshow("image", image)
cv2.resizeWindow("image", 700, 700) #reduce the dimensions of the original file to 50% (to fit to the screen)
cv2.waitKey(0)
