...
def calc_heuristics(destination): #destination -> str
    for key in heuristic: 
        if (key != destination):
            ...
            heuristic[key][0] = round(dist*0.65,2) #1px ~ 0.6 km; convert the distance in pixels to a distance in kilometers
...
while not stop:
    ...
    if (len(to_visit) != 0):
        ...
        #adds to dist_acc (distance accumulated) the distance to the next node
        dist_acc += curr["dist"][curr["adj"].index(to_visit[index_less])]
        #the next node assuming the position of the current (curr)
